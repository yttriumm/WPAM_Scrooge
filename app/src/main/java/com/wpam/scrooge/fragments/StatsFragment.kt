...
class StatsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        changePasswordButton.setOnClickListener {
            ...
            if(newPass.length < 6) {
                Toast.makeText(activity!!, "Hasło musi być dłuższe niż 6 znaków.", Toast.LENGTH_SHORT)
                        .show()
            }
            else if (newPass != confPass) {
                Toast.makeText(activity!!, "Hasła muszą się zgadzać.", Toast.LENGTH_SHORT)
                        .show()
            }
            else if (newPass == confPass) {
                ...
                user.updatePassword(newPass).addOnSuccessListener {
                    ...
                    transaction.commit()
                    Toast.makeText(activity!!, "Zmieniono hasło.", Toast.LENGTH_SHORT)
                            .show()
                }.addOnFailureListener {
                    Toast.makeText(activity!!, "Nie udało się zmienić hasła.", Toast.LENGTH_SHORT)
                            .show()
                }
